...
Trong Chương 1 và 2, chúng ta giải thích we explained how the `this` binding points to various objects depending on the call-site of the function invocation. But what exactly are objects, and why do we need to point to them? We will explore objects in detail in this chapter.

## Cú pháp

Object được hình thành từ hai dạng: dạng khai báo, hoặc dạng xây dựng.
...
Cả hai dạng trên đều có kết quả chính xác cùng một kiểu của object. Khác biệt duy nhất là bạn có thể thêm một hay nhiều cặp key/value trong khai báo thuần túy, trong khi dạng xây dựng bạn phải thêm từng thuộc tính một.

**Ghi chú:** Việc tạo object bằng dạng xây dựng rất cực kỳ ít. Bạn cũng luôn muốn tạo object theo dạng thuần túy. The same will be true of most of the built-in objects (see below).
