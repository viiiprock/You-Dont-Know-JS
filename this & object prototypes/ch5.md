...
Objects trong JavaScript có thuộc tính bên trong, được biểu thị đặc tả là `[[Prototype]]`, nó đơn giản chỉ là một tham chiếu đến đối tượng khác. Hầu như các object được cung cấp một giá trị không `null` cho thuộc tính này, tại thời điểm tạo ra chúng.
...
Tham chiếu `[[Prototype]]` dùng để làm gì? In Chapter 3, we examined the `[[Get]]` operation that is invoked when you reference a property on an object, such as `myObject.a`. For that default `[[Get]]` operation, the first step is to check if the object itself has a property `a` on it, and if so, it's used.
...
So, we have `myObject` that is now `[[Prototype]]` linked to `anotherObject`. Rõ ràng `myObject.a` không thực sự tồn tại, tuy nhiên, truy cập thuộc tính thành công (being found on `anotherObject` instead) and indeed finds the value `2`.
...
### Thiết lập & Shadowing Properties
...
Nếu `myObject` object đã có một thuộc tính `foo` accessor dữ liệu bình thường trực tiếp hiện diện trong nó, phép gán chỉ là đơn giản thay đổi giá trị của thuộc tính hiện hữu.

Nếu `foo` is not already present directly on `myObject`, the `[[Prototype]]` chain is traversed, just like for the `[[Get]]` operation. If `foo` is not found anywhere in the chain, the property `foo` được thêm trực tiếp `myObject` with the specified value, as expected.

However, if `foo` đã xuất hiện đâu đó trong chuỗi, nuanced (and perhaps surprising) behavior can occur with the `myObject.foo = "bar"` assignment. We'll examine that more in just a moment.

Nếu thuộc tính `foo` ends up both on `myObject` itself and at a higher level of the `[[Prototype]]` chain that starts at `myObject`, this is called _shadowing_. The `foo` property directly on `myObject` _shadows_ any `foo` property which appears higher in the chain, because the `myObject.foo` look-up would always find the `foo` property that's lowest in the chain.
...
1.  If a normal data accessor (see Chapter 3) property named `foo` is found anywhere higher on the `[[Prototype]]` chain, **and it's not marked as read-only (`writable:false`)** then a new property called `foo` is added directly to `myObject`, kết quả là một **shadowed property**.
