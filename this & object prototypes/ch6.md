...
But now let's try to think about the same problem domain, but sử dụng _ủy thác hành vi_ thay cho _classes_.
...
**Behavior Delegation** nghĩa là: cho object (`XYZ`) đưa ra một ủy thác (đến `Task`) cho tham chiếu thuộc tính hoặc phương thức không tìm thấy trong object (`XYZ`).
...
#### Nhân bản ủy thác (Không được phép)
